...
def test_object_acl_manager():
    ...
    oaclm = user_access.ObjectACLManager(
        'object_acls_collection', session_key, context.app, owner=context.owner,
    ...
    assert (type((obj_acl.obj_perms['read']) == list),f"{obj_acl.obj_perms['read']}"
    assert ((obj_acl.obj_perms['read']).sort() == ['admin', 'user1'])
...
def test_app_capability_manager():
    ...
    acm = user_access.AppCapabilityManager(
        'app_capabilities_collection', session_key, context.app, owner=context.owner,
        scheme=context.scheme, host=context.host, port=context.port)
...
def test_get_current_username():
    ...
    assert user_access.get_current_username(
        session_key, scheme=context.scheme,
        host=context.host, port=context.port) == context.username
...
def test_user_is_capable():
    ...
    assert not user_access.user_is_capable(
        session_key, context.username, 'test_capability',
        scheme=context.scheme, host=context.host, port=context.port)
